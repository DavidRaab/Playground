...
(*
Not every function call uses tuple syntax. But the call to create the `Vector2` is.
...
But I still have a problem reading the above line and it has all to do becuase Vector2
uses Tuple syntax. Let me show you what i mean: First we extract the `Vector2`.
...
when I don't do that the language will see the code like this.
...
This is still considered LISP-like. Even if the surrounding parenthesis are omitted. But
ML-Like languages (ML, F#, Haskell, ...) are allowed to omit these parenthesis if possible.

But still you see them around `(float32 x * 11f)`. And in my opinion it is much easier to see
