...
sub area($shape) {
    if    ( $shape->[0] == $is_square    ) { return $shape->[1] * $shape->[1] }
    elsif ( $shape->[0] == $is_rectangle ) { return $shape->[1] * $shape->[2] }
    elsif ( $shape->[0] == $is_triangle  ) { return $shape->[1] * $shape->[2] * 0.5 }
    elsif ( $shape->[0] == $is_circle    ) { return $shape->[1] * $shape->[1] * 3.141592654 }
    else {
        die "Unknown shape.\n";
    }
}
