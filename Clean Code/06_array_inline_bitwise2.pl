...
my $sum   = 0;
for my $shape ( @shapes ) {
    if    ( $shape->[0] & 1 ) { $sum += $shape->[1] * $shape->[1] }
    elsif ( $shape->[0] & 2 ) { $sum += $shape->[1] * $shape->[2] }
    elsif ( $shape->[0] & 4 ) { $sum += $shape->[1] * $shape->[2] * 0.5 }
    elsif ( $shape->[0] & 8 ) { $sum += $shape->[1] * $shape->[1] * 3.141592654 }
    else {
        die "Unknown shape.\n";
    }
}
